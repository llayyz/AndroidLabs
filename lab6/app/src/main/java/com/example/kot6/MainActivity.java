...
public class MainActivity extends AppCompatActivity {
    ...
    private List<String> downloadedJournals;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        downloadedJournals = new ArrayList<>();
        adapter = new ArrayAdapter<>(this, android.R.layout.simple_list_item_1, downloadedJournals);
        listViewJournals.setAdapter(adapter);

        listViewJournals.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                String selectedJournal = downloadedJournals.get(position);
                String journalId = selectedJournal.split(" ")[0];
                fileJournal = new File(getExternalFilesDir(null) + "/Journals/journal_" + journalId + ".pdf");
        ...
        buttonDelete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (fileJournal.exists()) {
                    ...
                    textJournalEmpty.setVisibility(View.VISIBLE);
                    layoutJournalReady.setVisibility(View.GONE);
                    downloadedJournals.remove(fileJournal.getName());
    ...
    private class DownloadFileTask extends AsyncTask<String, Void, Boolean> {
        ...
        protected void onPostExecute(Boolean result) {
            if (result) {
                ...
                downloadedJournals.add(fileJournal.getName());
                adapter.notifyDataSetChanged();
            } else {
                showAlertDialog(getString(R.string.alert_dialog_error), getString(R.string.alert_dialog_nofile));
                runOnUiThread(() -> {
    ...
    private void loadDownloadedJournals() {
        File folderJournal = new File(getExternalFilesDir(null), "Journals");
        if (folderJournal.exists() && folderJournal.isDirectory()) {
            File[] files = folderJournal.listFiles();
            if (files != null) {
                for (File file : files) {
                    if (file.getName().endsWith(".pdf")) {
                        downloadedJournals.add(file.getName());
                    }
                }
                adapter.notifyDataSetChanged();
            }
